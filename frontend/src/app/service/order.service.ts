...
import { OrderResponse, UpdateOrderStatusRequest, PlaceOrderRequest } from '../models/order.model';
...
export class OrderService {
    ...
    getMyOrders(params: PageableParams): Observable<Page<OrderResponse>> {
        ...
        return this.http.get<ApiResponse<Page<OrderResponse>>>(`${this.apiUrl}/my-orders`, { params: httpParams })
            .pipe(
    ...
    getGuestOrderByIdAndToken(orderId: number, guestToken: string): Observable<OrderResponse> {
        const url = `${this.apiUrl}/${orderId}`;
        // Gửi token qua query param
        const params = new HttpParams().set('token', guestToken);
        // Không cần header Authorization vì là khách
        return this.http.get<ApiResponse<OrderResponse>>(url, { params })
            .pipe(
    ...
    getOrderByIdAndUser(orderId: number): Observable<OrderResponse> {
        const url = `${this.apiUrl}/${orderId}`;
        console.log(`OrderService: Calling GET ${url} for logged-in user (token should be added by interceptor).`);
    ...
    getAllOrdersForAdmin(params: PageableParams): Observable<Page<OrderResponse>> {
        let httpParams = new HttpParams()
            .set('page', params.page.toString())
            .set('size', params.size.toString());
        if (params.sort) {
        ...
        return this.http.get<ApiResponse<Page<OrderResponse>>>(`${this.apiUrl}/admin/orders`, { params: httpParams })
            .pipe(
                map(response => this.extractPageData(response, params.size)), // Dùng hàm helper
                catchError(this.handleError)
            );
    ...
    updateOrderStatus(orderId: number, newStatusId: number): Observable<OrderResponse> {
        const url = `${this.apiUrl}/${orderId}/status`;
        const requestBody: UpdateOrderStatusRequest = { newStatusId };
        return this.http.put<ApiResponse<OrderResponse>>(url, requestBody)
            .pipe(
                map(response => this.extractOrderData(response)), // Dùng hàm helper
                catchError(this.handleError)
            );
    ...
    private extractOrderData(response: ApiResponse<OrderResponse> | null): OrderResponse {
        if (response && response.success && response.data) {
            ...
            console.log('OrderService - extractOrderData - Received raw orderData:', JSON.stringify(orderData));
            ...
            // Ném lỗi cụ thể hơn
            throw new Error(response?.message || 'Dữ liệu đơn hàng không hợp lệ hoặc API không thành công.');
        }
    }


    // Helper xử lý và trích xuất dữ liệu Page<OrderResponse>
    private extractPageData(response: ApiResponse<Page<OrderResponse>> | null, requestSize: number): Page<OrderResponse> {
        if (response && response.success && response.data && Array.isArray(response.data.content)) {
            ...
            const frontendPage: Page<OrderResponse> = {
                ...
                pageable: pageDataFromBackend.pageable ?? { sort: { sorted: false, unsorted: true, empty: true }, offset: pageDataFromBackend.number * requestSize, pageNumber: pageDataFromBackend.number, pageSize: requestSize, paged: true, unpaged: false },
                sort: pageDataFromBackend.sort ?? { sorted: false, unsorted: true, empty: true },
            ...
            if (!frontendPage.pageable) {
                const defaultSort: Sort = { sorted: false, unsorted: true, empty: true }; // Mặc định không sort
            ...
            console.warn('extractPageData: API response structure mismatch or data is missing or content is not an array.');
            // Trả về Page rỗng chuẩn theo interface frontend
            const defaultSort: Sort = { sorted: false, unsorted: true, empty: true };
            const defaultPageable: Pageable = { sort: defaultSort, offset: 0, pageNumber: 0, pageSize: requestSize, paged: true, unpaged: false };
            return {
                content: [], totalElements: 0, totalPages: 0, number: 0, size: requestSize, first: true, last: true, numberOfElements: 0, empty: true,
    ...
    private handleError(error: HttpErrorResponse): Observable<never> {
        ...
        if (error.status === 0) {
          ...
          displayMessage = 'Lỗi mạng hoặc không thể kết nối đến máy chủ. Vui lòng kiểm tra kết nối.';
          ...
          // Có thể tạo message chung chung hơn dựa trên status, nhưng message từ backend tốt hơn
          displayMessage = `Lỗi từ máy chủ (Mã: ${error.status}). Vui lòng thử lại sau.`;
          // Hoặc giữ nguyên message mặc định
        }
        ...
        // Component sẽ bắt được Error này và lấy `err.message`
